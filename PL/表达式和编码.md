...
* 字符数据 [https://cldr.unicode.org/index/downloads](https://cldr.unicode.org/index/downloads)
* ASCII表 [https://bestasciitable.com](https://bestasciitable.com)
* Unicode [https://blog.xinshijiededa.men/unicode](https://blog.xinshijiededa.men/unicode)


**Unicode 官方标准的大分类体系（通用类别 General Category）**

| 大类代码 | 大类名称 | 包含的子类说明（全 30 个子类） |
| :--- | :--- | :--- |
| **C (Other)** | 控制/其他 | • **Cc (Control)**：控制字符（C0/C1 控制区，如 NUL、换行、回车、ESC 等）<br>• **Cf (Format)**：格式字符（零宽字符、Bidi 方向控制符、软连字符、Tag 隐形标签）<br>• **Cs (Surrogate)**：代理项（`U+D800–DFFF`，UTF-16 成对编码，孤立代理常致解析崩溃）<br>• **Co (Private Use)**：私有使用区（PUA 各区，自定义字符，常用于字体反爬）<br>• **Cn (Unassigned)**：未分配码位（含保留码位以及永久非字符 Noncharacters） |
| **Z (Separator)** | 分隔符 | • **Zs (Space Separator)**：空格（包含普通空格、NBSP、全角空格等约 20 种空白）<br>• **Zl (Line Separator)**：行分隔符（仅 `U+2028`，曾引发内嵌 JSON 解析崩溃）<br>• **Zp (Paragraph Separator)**：段落分隔符（仅 `U+2029`，常致文本分段异常） |
| **M (Mark)** | 结合标记 | • **Mn (Nonspacing Mark)**：非间距标记（组合音标、变体选择子；易被堆叠成 Zalgo 文本破坏渲染）<br>• **Mc (Spacing Mark)**：间距结合标记（自身占宽度的组合符，印度系语言常见）<br>• **Me (Enclosing Mark)**：包围/环绕标记（如将字符包围的圆圈 `⃝`、方框 `⃞`） |
| **L (Letter)** | 字母/文字 | • **Lu (Uppercase Letter)**：大写字母（`A–Z` 以及西里尔、希腊大写字母等）<br>• **Ll (Lowercase Letter)**：小写字母（`a–z` 以及各类文字小写，存在同形混淆风险）<br>• **Lt (Titlecase Letter)**：词首大写字母（如复合字母 `Ǆ`）<br>• **Lm (Modifier Letter)**：修饰符字母（如上标音调 `ʰ`、`ʳ`）<br>• **Lo (Other Letter)**：其他文字（汉字、假名等表意文字；**特别注意：含伪装空白 `U+3164` HANGUL FILLER 及 `U+FFA0`**） |
| **N (Number)** | 数字 | • **Nd (Decimal Number)**：十进制数字（ASCII `0–9`、全角数字、阿拉伯-印度文数字等）<br>• **Nl (Letter Number)**：字母型数字（如罗马数字 `Ⅰ`、`Ⅱ`、苏州码子）<br>• **No (Other Number)**：其他形式数字（上下标数字 `²`、分数 `½`、带圈数字 `①` 等） |
| **P (Punctuation)** | 标点符号 | • **Pc (Connector Punctuation)**：连接标点（如下划线 `_`、全角下划线）<br>• **Pd (Dash Punctuation)**：破折/连字符（如破折号 `—`、普通短横线 `-`）<br>• **Ps (Open Punctuation)**：开启/前括号（如 `(`、`[`、`{`、`《`、`【`）<br>• **Pe (Close Punctuation)**：闭合/后括号（如 `)`、`]`、`}`、`》`、`】`）<br>• **Pi (Initial Punctuation)**：初引号（如法文书名号 `«`、各类前双/单引号）<br>• **Pf (Final Punctuation)**：末引号（如法文书名号 `»`、各类后双/单引号）<br>• **Po (Other Punctuation)**：其他标点（如逗号 `,`、句号 `.`、顿号 `、`、感叹号 `!` 等常规标点） |
| **S (Symbol)** | 符号 | • **Sm (Math Symbol)**：数学符号（如 `+`、`=`、`<`、`>`、`≠`、`√`）<br>• **Sc (Currency Symbol)**：货币符号（如 `$`、`€`、`¥`、`£` 等）<br>• **Sk (Modifier Symbol)**：修饰符符号（单独出现的重音符、扬抑符 `^` 等）<br>• **So (Other Symbol)**：其他符号（**包含绝大多数 Emoji 表情**、杂项图形符号、扑克牌、箭头等） |
...
+ [https://doc.visten.org/encoding-and-character-set/unicode-control-characters.html](https://doc.visten.org/encoding-and-character-set/unicode-control-characters.html)
...
```
* 正则引擎 [https://github.com/xindoo/regex](https://github.com/xindoo/regex)
* 正则基准 [https://github.com/burntsushi/rebar](https://github.com/burntsushi/rebar)
* 正则语言 [https://github.com/rulex-rs/pomsky](https://github.com/rulex-rs/pomsky)
* 正则测试 [https://github.com/gskinner/regexr](https://github.com/gskinner/regexr)
* 正则入门 [https://zq99299.github.io/note-book/regular](https://zq99299.github.io/note-book/regular)
* 正则入门 [https://deerchao.cn/tutorials/regex/regex.htm](https://deerchao.cn/tutorials/regex/regex.htm)
* 正则测试 [https://devtoolcafe.com/tools/regex](https://devtoolcafe.com/tools/regex)
* 正则生成 [https://regex.ai](https://regex.ai)
* 正则库 [https://github.com/asweigart/humre](https://github.com/asweigart/humre)
* 正则教程 [https://github.com/ziishaned/learn-regex](https://github.com/ziishaned/learn-regex)
* 正则引擎 [https://developer.aliyun.com/article/763503](https://developer.aliyun.com/article/763503)
* 正则引擎 [https://developer.aliyun.com/article/763502](https://developer.aliyun.com/article/763502)
* 正则引擎 [https://rhaeguard.github.io/posts/regex](https://rhaeguard.github.io/posts/regex)
* 正则性能 [https://github.com/rust-leipzig/regex-performance](https://github.com/rust-leipzig/regex-performance)
* 正则练习 [https://imageslr.github.io/regexone-cn/lesson/introduction_and_abcs.html](https://imageslr.github.io/regexone-cn/lesson/introduction_and_abcs.html)
* 正则可视化 [https://github.com/bowen7/regex-vis](https://github.com/bowen7/regex-vis)
* 正则表达式 [https://www.processon.com/view/link/5e3f7ca3e4b0a3daae81090f](https://www.processon.com/view/link/5e3f7ca3e4b0a3daae81090f)
* 正则解析 [https://github.com/hemashushu/js-regexp-interpreter](https://github.com/hemashushu/js-regexp-interpreter)
